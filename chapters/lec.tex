...
\[
S = \left\{ f : \mathbb{N} \to R \ \middle|\ f(r) = 0 \text{ for all but finitely many } r \right\},
\]
...
\[
\mathcal{N}(R) = \{\text{all nilpotent elements of } R\} \quad \leadsto \quad \text{the \emph{nil-radical} of } R.
\]

\begin{theorem}
\[
\mathcal{N}(R) = \bigcap_{P \in \operatorname{Spec} R} P, \qquad \text{where } \operatorname{Spec} R = \text{all prime ideals of } R.
\]
\end{theorem}

\begin{proof}
Let $x \in \mathcal{N}(R)$. Then for any $P \in \operatorname{Spec} R$, there exists $n_P \in \mathbb{N}$ such that
\[
...
Since $x \notin M$ and $M \in \operatorname{Spec} R$, we conclude $x \notin \bigcap_{P \in \operatorname{Spec} R} P$, completing the proof.
\end{proof}
...
\begin{proof}
Let $P \in \operatorname{Spec} R$. We have a natural morphism $\varphi: R[X] \to (R/P)[X]$ given by
\[
...
\]
That is, $a_0 \notin P$ and $a_1, \ldots, a_n \in P$. As $P \in \operatorname{Spec} R$ was arbitrary,
\[
a_0 \notin P \text{ and } a_i \in P \ \ \forall\, i \in [n], \qquad \text{for all } P \in \operatorname{Spec} R.
\]
...
\begin{enumerate}[label=(\arabic*)]
    \item $f$ is injective;
    \item $\operatorname{Im}(f)$ generates $F$ as an $R$-module.
\end{enumerate}
\end{proposition}
...
Let $A$ be the submodule of $F$ generated by $\operatorname{Im}(f)$. Since $f(s) \in \operatorname{Im}(f) \subseteq A$ for every $s \in S$, $f$ factors as $f = i_A \circ f^*$ for a set-theoretic map $f^*: S \to A$ and the inclusion $i_A: A \hookrightarrow F$:
\[
\begin{tikzcd}
...
\]
So $i_A \circ h$ and $\mathrm{id}_F$ both satisfy $(-) \circ f = f$; by uniqueness in the definition of freeness (applied with $M = F$, $g = f$), $i_A \circ h = \mathrm{id}_F$. Hence $i_A$ is onto, i.e.\ $A = F$, so $\operatorname{Im}(f)$ generates $F$.
\end{proof}

\begin{theorem}[Uniqueness of free modules]
...
\begin{theorem}[Existence of free $R$-modules]
